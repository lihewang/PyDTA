...
class tdsp:
    ...
    def __init__(self, num_nodes, link_attribute, args):
    ...
    def build(self, sp_task):
        ...
        self.parent_node = pd.Series(np.full(self.num_nodes,-1), index=range(self.num_nodes))
        ...
        while not self.my_heap.is_empty():
            ...
            if self.top_node_index == self.d_node_index:
                break
            #No next link
            if not self.top_node_index in self.link_attribute[:,0]:
                continue
            ...
            #Next links index           
            self.next_links = np.where(self.link_attribute[:,0] == self.top_node_index)           

            for self.i in self.next_links[0]:
...
if __name__ == "__main__":
    ...
    my_sp = tdsp(len(node_index), link_attribute.to_numpy(dtype='f'), args.to_numpy())
